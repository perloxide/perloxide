...
- **Promoted tier.**  A `Referent` — a shared, stably addressed,
  reference-counted identity holding a `Scalar`.  A value is
...
enum Value {
    ...
    // niche is PString's — 136 of its 256 tag values are used,
...
*mutability*, mirroring `Referent`'s Mut/Const split — because the
...
`cell::Referent` remains the API view type, reconstructed by `Arc`
...
aliases the loop variable to readonly elements, so `AliasConst`
slots are real.
...
promoted-alias variants (`AliasMut`/`AliasConst`) already carry
...
enum Value {
    ...
    String(PString),              // taint in the PString tag
...
and the `PString` tag bit; its placement rules, semantics,
...
**The scan-state inventory [DECISION].**  Eleven states, each an
assertion set; five are terminal (a completed classification,
eligible for the small tiers' envelopes), six are partial
knowledge.

- `Unknown` — asserts nothing.
- `Ascii` — all bytes `0x00`–`0x7F`.  Terminal.
- `Utf8Latin1` — Rust-valid, all code points ≤ `U+00FF`, at least
  one ≥ `U+0080`.  Terminal.
- `MaybeUtf8Latin1` — Rust-valid, all code points ≤ `U+00FF`; the
  non-Ascii witness is not asserted.  Narrows to `Ascii` or
  `Utf8Latin1`; a failed Ascii probe alone completes it to
  `Utf8Latin1`, both witnesses then in hand.
- `Utf8NonLatin1` — Rust-valid, at least one code point ≥
  `U+0100`.  Terminal.
- `ValidUtf8` — Rust-valid, nothing else.
- `Utf8NonAscii` — Rust-valid, at least one code point ≥
  `U+0080`.  Narrows to `Utf8Latin1` or `Utf8NonLatin1`.
- `ExtendedUtf8` — perl-decodable and Rust-invalid: contains a
  surrogate or a code point ≥ `U+110000` (each ≥ `U+0100`, so the
  beyond-Latin-1 range fact is derivable; the range predicates
  lean on it).  Terminal.
- `MaybeExtendedUtf8` — perl-decodable; Rust validity and range
  unasserted.  `is_perl_utf8_valid` answers true with no scan; a
  full classification of content honestly in this state can land
  on any perl-valid terminal but never `MalformedUtf8` — that
  outcome would falsify the assertion, so debug builds treat it
...
clean when neither end splits a sequence — two O(1)
continuation-byte tests, valid for the perl-extended forms too.
Character-indexed `substr` cuts clean by construction; byte-offset
slicing — Rust's, or perl's own on a flag-off string viewed as
raw bytes — can cut dirty, and a dirty cut of any
validity-asserting source is *proven* malformed by the cut:
birth `MalformedUtf8`, terminal and free.  Under clean cuts:

- `Unknown` → `Unknown`; `Ascii` → `Ascii` (exact; Ascii cannot
  cut dirty).
- `Utf8Latin1` → `MaybeUtf8Latin1` (the non-Ascii witness may be
  excluded; the range bound survives).
- `MaybeUtf8Latin1` → `MaybeUtf8Latin1`.
- `Utf8NonLatin1` → `ValidUtf8`; `Utf8NonAscii` → `ValidUtf8`
  (validity survives, the witness may not).
- `ValidUtf8` → `ValidUtf8`.
...
Below the slice-eager floor (4 KiB [DECISION] — distinct from the
64 KiB construction line because a slice's scan is full price
where construction's fuses with a copy) the subrange is
classified at birth instead: one cheap pass beats a scan per
descendant, since view envelopes cannot propagate narrowing.
...
u8`), and a private projection module derives `u8` constants from
the variants solely for pattern position, where a cast cannot
appear.  No numeric fact is stated twice, and the two `Maybe`
states are born unreachable — retained per the ruling above,
their births arriving with slicing.

`Inline` strings have no heap header; their scan state lives in the
tag — and needs only the five *terminal* states (`Ascii`,
`Utf8Latin1`, `Utf8NonLatin1`, `ExtendedUtf8`,
`MalformedUtf8`), because inline strings are scanned eagerly and
...
`Value`/`Scalar` layouts (§2.3.6) most of the discriminant
space.
...
A utf8-flagged string can therefore be scan-state `MalformedUtf8`
...
How `PString` represents perl-extended sequences that Rust `str`
...
1. *Length*: same flags and byte lengths differ ⇒ false (byte
   ...
   flagged side known non-ASCII *and* Rust-valid (`Utf8Latin1`,
   `Utf8NonAscii`) and byte lengths equal ⇒ false (at least one
...
2. *Same flags, both terminal, states differ* ⇒ false — the
   ...
   flag settings at once: `Utf8Latin1` vs
   `Ascii`/`Utf8NonLatin1`/`ExtendedUtf8`/`MalformedUtf8`, and
   every other differing terminal pair, including `ExtendedUtf8`
   vs `MalformedUtf8`.  Flagged pairs where one side is terminal
   Rust-invalid (`ExtendedUtf8`/`MalformedUtf8`) and the other is
   known Rust-valid (`ValidUtf8`/`Utf8NonAscii`) are
   also false: valid bytes cannot equal invalid bytes.
3. *ASCII vs known-non-ASCII* (any flag combination): one side
   `Ascii`, the other any of `Utf8Latin1`, `Utf8NonLatin1`,
   `Utf8NonAscii`, `ExtendedUtf8`, `MalformedUtf8`,
   `NonAscii` ⇒ false.
4. *Cross-flag disjointness*: if the flagged side is
   `Utf8NonLatin1`, `ExtendedUtf8`, or `MalformedUtf8`, the
   ...
   `MalformedUtf8`-classified string is a byte string whose
   characters are its bytes, and can equal a flagged
   `Utf8Latin1` string (unflagged `80` equals flagged `C2 80`).

Rows the grid deliberately leaves undecided (`Unknown`,
`ValidUtf8`, cross-flag `Utf8NonAscii` and `NonAscii`
...
the comparison is skippable, guaranteed false.  Eleven states:

| state                | meaning                                                                                                      |
|----------------------|--------------------------------------------------------------------------------------------------------------|
| `Ascii`              | entirely U+0000–U+007F                                                                                       |
| `Utf8Latin1`         | Rust-valid, entirely U+0000–U+00FF, non-ASCII                                                                |
| `MaybeUtf8Latin1`    | Rust-valid, entirely U+0000–U+00FF, may or may not be ASCII                                                  |
| `Utf8NonLatin1`      | Rust-valid, contains a character ≥ U+0100                                                                    |
| `ValidUtf8`          | Rust-valid; nothing further known (could narrow to any of the three above)                                   |
| `Utf8NonAscii`       | Rust-valid, known non-ASCII; Latin-1-range unresolved                                                        |
| `ExtendedUtf8`       | perl-decodable, Rust-invalid: contains a code point Rust rejects (a surrogate or ≥ U+110000), hence ≥ U+0100 |
| `MaybeExtendedUtf8`  | perl-decodable, may or may not be Rust-valid UTF-8                                                           |
| `MalformedUtf8`      | violates the encoding patterns; invalid for Rust *and* perl                                                  |
| `NonAscii`           | a high bit is present; validity and range unknown                                                            |
| `Unknown`            | completely unknown                                                                                           |

The five fully-scanned terminals (`Ascii`, `Utf8Latin1`,
`Utf8NonLatin1`, `ExtendedUtf8`, `MalformedUtf8`) are **mutually
exclusive**: classification assigns exactly one, so `Ascii` content
is *excluded* from `Utf8Latin1` by definition (`Utf8Latin1` means
...
(`Unknown` = 0 pinned by §2.2.6; then `Ascii` = 1, `Utf8Latin1`
= 2, `Utf8NonLatin1` = 3, `ValidUtf8` = 4,
`Utf8NonAscii` = 5, `ExtendedUtf8` = 6, `MalformedUtf8` = 7,
`NonAscii` = 8): Rust-valid ⟺ 1–5, perl-decodable ⟺ 1–6,
...
`ValidUtf8` short-circuits to `Ascii` or
`Utf8NonAscii` (validity retained, range deferred), and only
operations that genuinely need range (downgrade hashing, the
equality fast path) pay the full lead-byte pass, narrowing
`ValidUtf8`/`Utf8NonAscii` to a terminal.
...
**Perl-valid vs. Rust-valid UTF-8: the `ExtendedUtf8` state.**
...
The scan lattice therefore distinguishes them: `ValidUtf8`
means Rust-valid; **`ExtendedUtf8`** means perl-decodable but
Rust-invalid (necessarily non-ASCII); `MalformedUtf8` means
malformed under perl's extended rules too.  `as_str` (a Rust-view
question) excludes `ExtendedUtf8`; character-level operations on
flagged strings (length, iteration, case, regex) include it and
reject only `MalformedUtf8`.  Inline strings carry `Extended` as
...
- `is_ascii()` scan (high-bit scan, bails at the first high bit):
  `Unknown` → `Ascii` or `NonAscii`; `ValidUtf8` →
  `Ascii` or `Utf8NonAscii` (validity retained, range deferred).
- Range classification (lead-byte pass; run only by consumers that
  need range — downgrade hashing, the equality fast path):
  `ValidUtf8` → `Ascii`, `Utf8Latin1`, or
  `Utf8NonLatin1`; `Utf8NonAscii` → `Utf8Latin1` or
  `Utf8NonLatin1`.
- Full classification (`as_str()` on a scan miss,
  `is_perl_utf8_valid()`): `Unknown`/`NonAscii` → any terminal
  (`Ascii` — not from `NonAscii` — `Utf8Latin1`,
  `Utf8NonLatin1`, `ExtendedUtf8`, `MalformedUtf8`).
...
high-bit probe yields `NonAscii` while a full classification
yields `Utf8Latin1` — and a plain last-wins store would widen the
...
- **Appending or prepending `&str`** to a valid UTF-8 string:
  yields a Rust-valid result: `ValidUtf8` in general, or
...
- **Inserting mid-string** into any string results in invalid
  ...
  — transition to `MalformedUtf8`.  This applies to
  `ExtendedUtf8` as much as to the Rust-valid states (extended
  sequences split just as badly).  The check can be skipped for
  `Ascii` (insertion point is guaranteed ASCII) and for
  `MalformedUtf8`/`Unknown` (no validity claim to protect).
...
- **Adding valid UTF-8 to valid UTF-8** preserves validity, and the
  ...
  → `Utf8Latin1`; `Utf8Latin1` + non-Latin-1 content →
  `Utf8NonLatin1`; `Utf8NonLatin1` + anything valid stays
  `Utf8NonLatin1`; any + `ValidUtf8` content →
  `ValidUtf8` (validity kept, range surrendered).
  `Utf8NonAscii` + ASCII or Latin-1 content stays
  `Utf8NonAscii`; + non-Latin-1 content → `Utf8NonLatin1`;
  `ValidUtf8` + known non-ASCII valid content →
  `Utf8NonAscii` (the addition proves non-ASCII).  The
  added content's range is knowable cheaply (the same lead-byte
  test).  Added to an `ExtendedUtf8` string, perl-decodable content
  of any kind preserves `ExtendedUtf8` — the Rust-rejected code
...
- **Removing valid UTF-8 characters** from a valid UTF-8 string
  ...
  knowledge weakens to `ValidUtf8` (from `Utf8NonAscii`
  and `Utf8NonLatin1` alike, and from `Utf8Latin1` too: removal
...
- **Removing non-ASCII bytes**: reset to `Unknown` (remaining
  content is unknown).  This includes removal from `ExtendedUtf8`
...
- `Heap` from Rust `&str` or `String`: set `ValidUtf8`
...
A compact value is promoted to the aliasing form
(`AliasMut`/`AliasConst`) when any of these occur:
...
- **Inline, ≤ 15 payload bytes**, in two length families: a
  ...
  **The storage types are the normative vocabulary.**  The
  ...
  §2.2.4 lattice terminal `MalformedUtf8` are the same
...
| **packed payload, aligned `PString`**    | **16** |
...
alignment making offset 8 a real boundary comes from `PString`
...
Knock-on layout targets, measured at implementation: `Scalar`
...
`Hash`'s map is an
`IndexMap<PString, Value>`.  Perl's `each` contract needs a
...
#### 2.2.12 `Array`: the front-gap engine [DECISION]:

`Array`'s slot storage is a front-gap engine shaped on perl's
...
**The spill arm [DECISION]:** `Array`'s representation is a
...
**The bucket engine.**  Entries are `(PString, Value)` pairs
...
constructed by `Hash::immutable()`: a persistent HAMT
(`imbl::HashMap`, Arc-backed) whose clones are O(1) root shares
and whose mutations path-copy O(log32 n) nodes.  `snapshot()`
exists on every `Hash` and returns
`Result<Hash, ScalarError>`: on this engine an O(1) clone —
...
```rust
enum Referent {
    Mut(Arc<RwLock<Scalar>>),   // ordinary mutable scalar
...
#### 2.3.2 `Scalar` — the mutable interior:
...
`Scalar` is 16 bytes (§2.2.9): the `Full` variant is a single pointer that
...
word, and the 16-byte `Scalar` — compares against perl 5
...
(`static LazyLock<Referent>`), constructed once as `Const` cells:
...
- The diagnostics architecture, crate-wide [DECISION]: perl's
  ...
  `Warning` trait: perl-exact `Display` of the body;
...
The payload is a `HeapArc<DualPayload>` holding a `PString` and a
...
flag-blind byte comparison is correct.  `PString` implements
...
const _: () = assert!(size_of::<Option<Scalar>>() == 16);
const _: () = assert!(size_of::<PString>() == 16);  // layout-compatible with Value's string region
```
...
- **Padding placement follows variant size.**  Sub-maximal variants
  ...
  type's discriminant —
  adding outer `Value` variants that carry `PString` defeats the
...
- **A survey of the string-crate ecosystem** (several dozen
  ...
  inline-over-Arc shape but is a whole-`PString` competitor, not
...
```text
PString    a Perl string: bytes + utf8/warned/tainted in its tag
...
Referent     shared identity of a promoted scalar (Mut | Const)
Scalar    mutable cell interior (Plain | Full), upgraded in place
...
`PerlStringSlot` (subsumed by `PString::Inline`), and the
standalone small-string maximum (now the `PString` inline
constant).
...
(`Referent`, `ArrayRef`, `HashRef`, ...), whose operations
...
node class; `Scalar` at 32-byte slots yields 128 per page).
...
- **`class_id`** — a **lazily allocated per-page parallel array**
  ...
  pointer *is* the unblessed answer.  `Scalar` pages in
...
Scalar                    8     8       16     32   PVMG 72
Array                     8     8       24     40   AV   64
Hash                      8     8     ~48+   ~64+   HV   56
```
...
`Scalar` node — perl's bodyless SV head, payload included —
...
finish DESTROY (the COMMIT CAS — a remove-$self-then-inspect
                  ...
                  release):
  Let k = the observed total of encoded strong units
        = hold + the temporary $self + resurrecting real refs.

  DestroyingNormal / DestroyingCleanObjsChecked:
    CAS (Destroying, k=2, H) → (CommittedDead, 1, H) — removes
                  the $self unit and commits death in one step.
    success:      rebless loop already done → clear class_id →
                  commit logical weak death: CommittedDead makes
                  every HeapWeak upgrade fail, so Perl-visible
                  weak values behave as plain undef — no
                  backreference list exists to walk (§2.4.8) →
                  release hold → payload drops (shallow; edges
                  already extracted — §2.4.9).  There is no
                  interval where only the hold remains yet
                  upgrades are still accepted.
    k > 2         (resurrection references exist):
      DestroyingNormal (RUN / END): CAS loop →
                  (Live, k-1, H) — the subtraction removes
                  exactly the $self unit; the hold and the
                  resurrecting references remain; retried
                  against fresh k while upgrades race.  Hold
                  re-arms (perl runs DESTROY again on the next
                  death — verified).
      DestroyingCleanObjsChecked: → (FatalCleanObjs, k, H): an
                  explicit exceptional state that owns the hold,
                  the $self unit, the resurrecting references,
                  and the finalization obligation.  The
                  perl-exact fatal error is the observable
                  consequence ("DESTROY created new reference to
                  dead object '%s'" — sv.c, check_refcnt &&
                  PL_in_clean_objs); discharge of the owned
                  units follows the ledgered fatal-cleanup
                  policy (§2.4.11) — "fatal" names the Perl
                  behavior, not a complete memory-state
                  transition.
    DestroyingGlobalForced: no resurrection check.  The rebless
                  ...
                  strip runs after the loop, in this order —
                  the reverse counterpart of first blessing's
                  install-hold-then-publish-class:
      1. Complete the rebless/DESTROY loop, still
                  DestroyingGlobalForced (nonclaimable).
      2. Release-store class_id = 0.
      3. One CAS removing the $self unit AND the hold together,
                  selecting by the observed k (retry loop against
                  weak upgrades; class_id stays zero throughout):
         k == 2:  → (CommittedDead, 0, no HOLD); every HeapWeak
                  upgrade fails from here — logical weak death,
                  no backreference list to walk
         k > 2:   → (Live, k-2, no HOLD) — alive, unblessed;
                  weak references remain upgradeable because the
                  referent remains Live; no further DESTROY can
                  occur (no hold, no class)
                  A reader that observes hold-free Live therefore
                  also observes the cleared class identity; a
                  retained hidden unit would leak the node
                  permanently.
```
...
Every reference-count operation gets the same state-sensitivity
(the machine is not complete until clone is covered, not only
upgrade).  `HeapArc::clone` from an existing real handle:

```text
Live (± HOLD)               allowed
GlobalForcedPending         allowed (real handles may exist)
Destroying*                 allowed ($self and other real handles)
FatalCleanObjs              owned by the fatal-cleanup policy
FinalizationPending /
  CleanObjsPending          no real handle exists to clone from —
                            the states are hold-only by
                            construction (debug-asserted)
CommittedDead               unreachable from a valid real handle
Tombstoned-domain survivor  ownership clone allowed; semantic
                            access is rejected by the checked
                            handles, not by the count layer
```

Clone and every other arithmetic operation increments or
decrements the count bits without altering state flags.
`downgrade`, `strong_count`, and `ptr_eq` are unconditional on any
live handle; the internal $self temporary is created by the
drainer inside the pending→destroying claim, never by public
clone.
...
PageMeta (in the extent descriptor region)
    ...
    ops: &'static NodeOps
        ...
        extract_edges          (moves every strong child out,
                                into the release worklist)
...
because the worklist alone does not prevent recursion — the
normative invariant (§2.4.9): **no graph-bearing payload reaches
Rust payload destruction while it still owns a strong Perl graph
edge.**  Every death path preserves it: ordinary last drop,
non-resurrecting `DESTROY`, level-1 demolition, forced-sweep
death, and post-tombstone cleanup all extract edges first, then
drop a payload whose remaining destructor is shallow.  (The
...
with the contract explicit: `Drop` guarantees *publication*, not
callback completion; both polling calls may run arbitrary Perl
code and may recurse; a host with no active interpreter either
polls or accepts retention; domain tombstoning suppresses all
later Perl callbacks; callback thread selection is unspecified
until §13; and no ordering is guaranteed between external and
interpreter-originated finalizations.
...
The last-real-drop algorithm maps every phase to a release mode
— nothing is implicit:

```text
Phase                          blessed last real drop
Running                        FinalizationPending (normal mode)
EndPhase                       normal mode; resurrection allowed
EnteringGlobalDestruction      normal mode — perl's equivalent
                               window (PL_phase = DESTRUCT before
                               sv_clean_objs) has PL_in_clean_objs
                               false, so ordinary resurrection
                               rules apply (probe-verifiable)
CleaningObjects, checked pass  CleanObjsPending
CleaningObjects, during forced
  callbacks                    CleanObjsPending (an unreached
                               node is still Live — §2.4.6 step 4)
Demolishing                    no Perl finalization; mechanical
Tombstoned                     no Perl finalization; mechanical
```

Context-free host drops follow the same table through the
producer protocol's phase validation; unblessed nodes take the
mechanical path in every phase.
...
At level 0 the distinction between *ownership liveness* and
*semantic domain liveness* is load-bearing and explicit in the
checked-handle API: a weak upgrade to a surviving unblessed node
may succeed at the ownership layer while every semantic operation
on the resulting handle is rejected because the domain is
tombstoned; a later last real release runs no Perl and cleans up
mechanically.

The passes mirror `sv_clean_objs` (source-verified structure):

1. Quiesce ordinary Perl execution; acquire slab enumeration
   guards (pages stable while enumerators walk).  Page guards
   keep page *memory* alive — they do not keep an individual
   payload alive against a concurrent context-free last drop of
   an *unblessed* node (blessed nodes are protected by their
   holds; unblessed ones have none).  Payload access therefore
   ...
   protocol covers every enumerator: the checked pass, the
   forced-sweep scan, level-1 demolition, diagnostic
   enumeration, and the future on-demand cycle detector.
   (Recorded alternatives: an exclusive domain operation gate —
   rejected for taxing every ordinary clone/drop — and an
   epoch/hazard scheme — deferred as the more complex escalation
   if pin contention ever measures.)  Without a payload claim,
   slab enumeration is not memory-safe.
2. **Checked reference-severing pass.**  Visit every strong
   ...
   the visited node's lock.  Because the edge is extracted before
   any callback runs, the aliasing cases are defined for free: a
   container referring to itself, multiple slots referring to one
   blessed node, and a `DESTROY` reaching back into the container
   its reference was removed from all observe the
   already-severed state.  Recursive `DESTROY` mutates the heap
   mid-pass; the traversal follows perl's finite arena-pass
   structure rather than promising rescans of modified nodes —
   actual coverage equivalence is the ledgered fixed-point probe
   item.  The normative criterion: **every field whose ownership
   contributes to a Perl referent's real strong count is visited
   or explicitly exempted** — scalar reference payloads, array
   elements, hash values, and later magic chains, closure
   environments, glob and IO fields, code objects, runtime caches
   that own values, and interpreter roots; `class_id` is exempt
   as a table identifier, not a `HeapArc` edge.  A per-type audit
   table accompanies the implementation.  Incomplete slot
   visitation is a
...
3. The named-root/glob analogs of perl's two glob passes, when
   globs exist.
4. **Forced sweep — incremental, never preclaiming.**  The scan
   proceeds one node at a time in arena order: find the next
   still-blessed `Live` node; claim *that node alone* through its
   existing hold — `Live → GlobalForcedPending`, no count
   contamination (an ordinary pin would be indistinguishable
   from a resurrecting reference at the post-`DESTROY` check);
   per ticket exclusivity (§2.4.5) the scan skips pending,
   destroying, and committed states.  Process the claimed node as
   `DestroyingGlobalForced` (`DESTROY` runs, the blessing is
   stripped regardless of retained references, §2.4.4), drain
   all nested finalization the callback generated, and only then
   continue scanning.  Batch-preclaiming even two nodes is a
   fidelity bug: if A and B were both preclaimed and A::DESTROY
   released B's last real reference, B would die in forced mode —
   where perl's corresponding decrement lands in *checked*
   destruction, with fatal resurrection semantics.  A node not
   yet reached remains `Live`, so a real-zero during another
   destructor correctly becomes `CleanObjsPending`.  A race with
   a concurrent ordinary release resolves by whichever CAS wins —
   one ticket either way.
...
6. **External-producer synchronization.**  Host threads are
   ...
   transition — bracketing only the push would let shutdown
   observe zero producers between a node's pending CAS and its
   publication, stranding an unpublished ticket.  Normative
   sequences:

   Producer: (1) increment `active_producers`
   ...
   Shutdown: (1) Release-publish the new phase; (2) Acquire-wait
   for old-phase producers to drain to zero; (3) drain tickets;
   (4) rescan the relevant nodes; (5) recheck the producer count
   and queue emptiness; (6) proceed only when the completion
   predicate is stable.  The Loom model includes producers that
   begin immediately before and immediately after each phase
   publication.
...
participates in domain accounting/enumeration.  `Scalar`,
`Array`, `Hash`, and `ConstScalar` (its payload can carry
graph edges) qualify, as will closures, globs, and IO.  `PString`
...
a Loom model** covering, at minimum: last real drop racing weak
upgrade; the completion commit racing weak upgrade; normal
resurrection with repeated count changes; forced unblessing
...
exactly once); finalizer holds, Perl weak-copy semantics,
tombstoning, and global destruction get state-machine oracle
tests instead — on randomized operation
...
The first slab migration is `Array` (the simplest graph-bearing
node), validating layout, pinning, reuse, finalization queueing,
and cross-thread races before `Scalar` — the most semantically
loaded type — moves.
...
state); ticket posture under exceptional shutdown, and the full
fatal-cleanup specification (whether cleanup continues for other
objects, whether remaining tickets drain or abandon, domain
poisoning, host-API surfacing, and the disposition of the
drain-owner token and producer barrier — the discharge rules for
`FatalCleanObjs`'s owned units); the exact rebless-loop algorithm
(class capture before callback, revisiting previously processed
classes, unblessing mid-loop, repeated reblessing, method-lookup
failure, stash tombstoning during `DESTROY` — source- and
probe-driven); the nested-`DESTROY` dynamic context under
owner-recursive draining ($@ per depth, `caller`, inherited
locals, a nested fatal checked error, unwinding the owner depth
across a Perl exception); the `rc_state` bit allocation document
(count width, hold encoding, immortal representation, overflow
threshold and abort, why arithmetic never touches flag bits —
tested near boundaries with a reduced-width model); the
weak-count conventions (implicit weak unit, downgrade,
last-strong release, freelist return, immortals, hold-only
nodes); debug-serial semantics (never an ownership proof, never
a stable Perl identity); the release-context
...
trait Magic: Send + Sync {
    fn mg_get(&self, interp: &mut Interpreter, sv: &Referent) -> Result<()>;
    fn mg_set(&self, interp: &mut Interpreter, sv: &Referent) -> Result<()>;
    fn mg_clear(&self, interp: &mut Interpreter, sv: &Referent) -> Result<()>;
    fn mg_free(&self, interp: &mut Interpreter, sv: &Referent) -> Result<()>;
...
payloads (§2.3.6), the `PString` tag for strings (§2.2.3), the
...
regex engine's capture-materialization path (§11) and `Hash`'s
...
- `Tainted` is monotonic: constructors produce clean values, the only
  ...
  on `use locale`), and `Hash` key canonicalization untaints
...
key on strings and `Hash` is keyed by `PString`, which cannot
...
`PString` one: comparing two strings numerically must numify, and
numifying is where a fatal warning can die.  A `PString::num_cmp`
...
dropped.  Over `Value` and `Scalar` an associated error type
distinguishes nothing, both being `PerlError`; `PString` does not
...
parsing code resolves the semantic ambiguity based on its own
position in the grammar — `Token::Slash` in `parse_term` means
regex, in `lex_operator` means division.  No shared disambiguation
state exists between the lexing and parsing layers.

Tokenization and parsing methods are all on the same `Parser`
struct, organized into sibling source files (`lexer.rs` and
`source.rs` for tokenization, `parser.rs` for grammar).  The
crate boundary is at the AST level.  The tokenization methods
perform context-sensitive scanning when called (heredoc initiation,
prototype scanning, substitution body partitioning), but do not
consult any disambiguation state — there is no `Expect` enum or
term-vs-operator flag.  Tokenization does read pragma and feature
state (`utf8_mode`, `features`) directly from `Parser` fields,
since these affect lexical behavior (Unicode identifier acceptance,
NFC normalization, paired delimiter availability).
...
`Expect` enum and no shared disambiguation state between the
lexing and parsing layers.  Instead, the parsing code resolves every
ambiguous token by virtue of knowing where it is in the grammar:

**`/` (regex vs division).**  The Pratt expression parser calls
`parse_term()` when expecting a value and `lex_operator()` when
looking for an infix operator.  `parse_term()` handles `Token::Slash`
as a regex start (calling `self.lexer.lex_body_str('/', true)` to
scan the pattern body).  `lex_operator()` maps `Token::Slash` to
...
(with optional flags like `//gi`).  In `lex_operator()`, it's the
...
ultimately initializes the runtime `PString` with the same
...
The lexing layer exposes a structured API for the parsing layer.
Methods fall into three categories: tokenization (three methods,
one per grammatical position), consumption (conditional token
matching), and construct-specific scanning.  There is no shared
mutable disambiguation state between the two layers — the parsing
code communicates context by choosing which method to call.  A
one-token lookahead cache (`self.current`) holds the
position-independent lex; `peek_token()` fills it and
`next_token()` consumes it, and the position-committed tokenization
methods reinterpret or re-lex the cached token for the few ambiguous
cases.
...
**`lex_operator(min_prec) -> Result<Option<(OpInfo, Spanned)>, ParseError>`**
— operator-position tokenization with precedence gating.  Fills the
one-token cache via position-independent `lex_token()` if empty,
maps the cached token to its `OpInfo` (precedence and
associativity), and gates on `min_prec`.  Three outcomes collapse
into `Option`:

- Not an operator token → returns `None`, leaving the cache intact.
- Operator but `left_prec < min_prec` → returns `None`, leaving the
  cache intact.  The operator is left for the outer Pratt loop level.
- Operator with `left_prec >= min_prec` → consumes the token and
  returns `Some((info, op_tok))`.
...
The operator table itself lives in `op_info_for_token()`, an
associated function keyed on the token and a snapshot of the
SmartMatch feature bit.  `lex_operator()` is its sole caller.  The
chain and trailing-non-associative lookaheads in `parse_operator`
also use `lex_operator()` rather than a separate peek method —
after `parse_expr(right_prec)` has consumed everything above, the
`>= min_prec` gate effectively tests exact precedence.

Most operator-position tokens are reused from the cache unchanged:
the position-independent lex of `/` is already `Slash` (division),
`.` is `Dot` (concatenation), and so on.  The one fix-up is the `x`
...
After consuming, peeks at the next non-whitespace byte:

- `)`, `]`, `}`, `;`, or EOF → returns `false` (trailing comma)
- anything else → returns `true` (more content follows)

If no comma is present at all, returns `false` without consuming.

The caller cannot distinguish "no comma" from "trailing comma" and
does not need to — both mean "no more elements follow."  This
single method replaces all comma-handling logic in the parser:
consecutive commas (`1, , , 3`), trailing commas (`1, 2,`), and
mixed comma/fat-comma separators (`a => 1, b => 2`).

The one context where `consume_comma()` is not appropriate is
subroutine signatures (`use feature 'signatures'`), where only `,`
is accepted and `=>` is a syntax error.  Signature parsing uses
`consume(&Token::Comma)` for strict comma matching.  Prototypes
are not affected — they are scanned as raw strings by
`lex_body_str()` and never tokenized.

**`expect(expected: &Token) -> Result<Spanned, ParseError>`** —
consumes the next token if it matches `expected`; otherwise returns
a parse error.  Used for required structural tokens like closing
delimiters.

#### 5.9.3 Construct-specific scanning methods:

These methods are called by the parser for constructs that require
specialized lexer behavior beyond simple tokenization.  They are
not replaced by `lex_term()` because they are construct-specific
rather than position-dependent:

- `lex_body_str(delim, raw)` — scan a delimited string body
  (prototypes, tr/// bodies).  Prototypes are scanned as raw
  strings, not tokenized — the parser extracts the known special
  characters (`$`, `@`, `%`, `&`, `*`, `\`, `+`, `_`, `;`) and
  ignores everything else.
- `start_subst_replacement(delim)` — partition a substitution
  replacement body and extract flags
- `scan_adjacent_word_chars()` — collect regex/subst modifier flags
  immediately after the closing delimiter
- `try_autoquoted_bareword_subscript()` — check for `$hash{word}`
  and return the bareword as a string if present

#### 5.9.4 Pragma and feature synchronization:

The parser syncs lexically-scoped state to the lexer at pragma
change points (`use`/`no` declarations, block exit):

- `set_utf8_mode(bool)` — controls Unicode identifier scanning,
  NFC normalization, and the `effective_utf8` fast path (§5.8.3)
- `.features = Features` — controls feature-gated keywords and
  Unicode paired delimiter availability

The lexer reads these fields during tokenization but never modifies
them — the parser is the sole authority on pragma state.
...
The parser holds a one-token lookahead cache (`self.current`),
filled by position-independent `lex_token()`.  `lex_term()` and
`lex_operator()` reinterpret the cached token for their position when
it is one of the few ambiguous cases, and reuse it directly
otherwise.
...
`parse_operator(left, info, op_tok)` — called when `lex_operator()`
has consumed an infix/postfix operator.  Receives the operator's
`OpInfo` and the already-consumed token.  Dispatches on the token:
...
fn parse_expr_continuation(
    ...
    while let Some((info, op_tok)) =
            self.lex_operator(min_prec)? {
        left = self.parse_operator(left, info, op_tok)?;
    }
    Ok(left)
}
...
`lex_operator(min_prec)` (§5.9.1) handles the entire check-and-
consume cycle: it maps the cached token to an `OpInfo`, checks
`left_prec >= min_prec`, and either consumes and returns
`Some((info, op_tok))` or returns `None` leaving the cache intact.
The chain and non-associative lookaheads inside `parse_operator`
also call `lex_operator` directly — after parsing the RHS at
`right_prec`, the only pending operator `lex_operator(info.prec)`
can match has exactly that precedence.
...
The parsing layer communicates with the lexing layer through
explicit method calls.  There is no shared mutable disambiguation
state between the two layers and no `Expect` enum — the call site
(`lex_term` vs `lex_operator`) determines how an ambiguous token
is read, rather than a flag the tokenization code consults.  A
one-token cache (`self.current`) holds the position-independent
lex; the position-committed methods reinterpret or re-lex it only
for the few ambiguous cases.
...
for the Pratt operator loop, and by the chain and non-associative
lookaheads inside `parse_operator`.  Returns `Some((info, op_tok))`
if the next token is an operator with `left_prec >= min_prec`,
consuming it; returns `None` otherwise, leaving the cache intact.
Handles the `x` repeat special case (splitting `x5` into operator
`x` and operand `5` in operator position).  The operator table
lives in `op_info_for_token()`.
...
- `lex_body_str(delim, raw)` — scan a delimited string body
  (prototypes are scanned as raw strings, not tokenized)
...
position.  In scalar slots, `consume_comma()` still handles
...
**Heredoc vs shift-left.**  In term position, `lex_term()` handles
this directly — it inspects the bytes after `<<` and either
...
The default Perl scalar is a `PString`-backed container with
...
```perl
let s: String = "hello";
my $x = s;                 # $x becomes a PString with UTF-8 flag set
...
```perl
my $raw = read_file("data.bin");       # PString, may not be valid UTF-8
...
| From                                 | To                        | Cost                  |
|--------------------------------------|---------------------------|-----------------------|
| `String` → `PString`                 | Wrap as Bytes, set flags  | Cheap (refcount)      |
| `PString` (Rust-valid) → `String`    | Take bytes, no validation | Cheap (flag check)    |
| `PString` (unknown) → `String`       | Full UTF-8 validation     | O(n), may fail        |
| `PString` → `Bytes`                  | Access inner `buf`        | Zero-cost             |
| `Bytes` → `PString`                  | Wrap with flags           | Cheap (refcount bump) |
...
```perl
let mut msg: String = "Hello";
msg .= ", world!";         # emits: String::push_str — no PString alloc
```
...
2. `PString` — the two storage kinds over `CowBuffer`, the tag
...
4. `Referent`, `Scalar`, `FullScalar`, `ConstScalar` — the
...
12. `Array` slab migration — the first node class through the
    new backend, validating layout, pinning, reuse, finalization
    queueing, and cross-thread races.

13. Remaining node classes (`Hash`, `Scalar`,
...
fn payload_stays_authoritative_through_coercion() {
    ...
    let mut cell = Scalar::from_float(3.7);
...
1. **`Arc<RwLock<T>>`-based value representation.**  Self-managing
   reference-counted values, no centralized arena.  `PString` as
