...
fn ordering_matches_perl_for_every_flag_combination() {
    // Pairs from `cmp` in container perl 5.44: the four flag combinations, ASCII and high octets, multi-byte sequences,
    // embedded NULs, empty strings, and packed content — including pairs whose bytes are identical but whose flags make
