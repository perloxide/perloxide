...
fn collect_lines(src: &str) -> Vec<String> {
    ...
    while let Ok(Some(line)) = lexer.next_line(false) {
...
fn empty_source() {
    ...
    assert!(matches!(lexer.next_line(false), Ok(None)));
}

#[test]
fn single_line_no_newline() {
    let mut lexer = Lexer::new(b"hello");
    let line = lexer.next_line(false).unwrap().unwrap();
...
fn line_numbers() {
    ...
    assert_eq!(lexer.next_line(false).unwrap().unwrap().number, 1);
    assert_eq!(lexer.next_line(false).unwrap().unwrap().number, 2);
    assert_eq!(lexer.next_line(false).unwrap().unwrap().number, 3);
}

#[test]
fn byte_offsets() {
    let mut lexer = Lexer::new(b"ab\ncde\nf\n");
    let l1 = lexer.next_line(false).unwrap().unwrap();
    assert_eq!(l1.offset, 0);
    let l2 = lexer.next_line(false).unwrap().unwrap();
    assert_eq!(l2.offset, 3);
    let l3 = lexer.next_line(false).unwrap().unwrap();
...
fn lexer_line_peek_and_advance() {
    ...
    let mut line = lexer.next_line(false).unwrap().unwrap();
...
fn heredoc_basic() {
    ...
    let decl = lexer.next_line(false).unwrap().unwrap();
    ...
    let body = lexer.next_line(false).unwrap().unwrap();
    assert_eq!(&body.line[..], b"hello");

    // Next line: terminator → None.
    assert!(lexer.next_line(false).unwrap().is_none());

    // Next line: saved remainder (the declaration tail).
    let remainder = lexer.next_line(false).unwrap().unwrap();
    ...
    let after = lexer.next_line(false).unwrap().unwrap();
...
fn heredoc_unterminated() {
    ...
    // Read body lines.
    lexer.next_line(false).unwrap().unwrap(); // hello
    lexer.next_line(false).unwrap().unwrap(); // world

    // EOF without terminator → error.
    assert!(lexer.next_line(false).is_err());
}
...
fn heredoc_stacked() {
    ...
    // A's body.
    let body_a = lexer.next_line(false).unwrap().unwrap();
    ...
    // B's body.
    let body_b = lexer.next_line(false).unwrap().unwrap();
    ...
    // Remainder restored: ");"
    let remainder2 = lexer.next_line(false).unwrap().unwrap();
...
fn heredoc_non_indented_inside_indented() {
    ...
    let inner_body = lexer.next_line(false).unwrap().unwrap();
...
fn ascii_only_heredoc_body_lines() {
    ...
    let body1 = lexer.next_line(false).unwrap().unwrap();
    assert!(body1.ascii_only, "first heredoc body line is ASCII");

    // Second body line: has UTF-8.
    let body2 = lexer.next_line(false).unwrap().unwrap();
...
fn peeked_heredoc_terminator_stays_pending_until_consumed() {
    ...
    assert!(matches!(lexer.next_line(true), Ok(None)));
    ...
    let restored = lexer.next_line(false).unwrap().unwrap();
    assert_eq!(&restored.line[..], b"saved");

    let rest = lexer.next_line(false).unwrap().unwrap();
...
fn push_back_precedes_underlying_source() {
    ...
    let first = lexer.next_line(false).unwrap().unwrap();
    ...
    let second = lexer.next_line(false).unwrap().unwrap();
